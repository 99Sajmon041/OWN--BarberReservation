...
public sealed class ReservationMappingProfile : Profile
{
    public ReservationMappingProfile()
    {
        CreateMap<BarberReservation.Domain.Entities.Reservation, ReservationDto>()
            ...
            .ForMember(x => x.ClientFullName, opt => opt.MapFrom(x => x.Customer != null ? $"{x.Customer.FirstName} {x.Customer.LastName}" : x.CustomerName))
            .ForMember(x => x.ClientEmail, opt => opt.MapFrom(x => x.Customer != null ? x.Customer.Email : x.CustomerEmail))
            .ForMember(x => x.ClientPhone, opt => opt.MapFrom(x => x.Customer != null ? x.Customer.PhoneNumber : x.CustomerPhone))
