    ...
    IUnitOfWork unitOfWork) : IRequestHandler<CreateAdminReservationCommand, int>
{
    public async Task<int> Handle(CreateAdminReservationCommand request, CancellationToken ct)
    {
        ...
        if (workingHours is null)
        {
            ...
            throw new DomainException("Kadeřník nemá pro tento den nastavenou pracovní dobu.");
        }
        ...
        if(hasTimeOff)
        {
            ...
            throw new DomainException("Kadeřník má v tomto čase volno.");
        }
        ...
        var reservation = new BarberReservation.Domain.Entities.Reservation
        {
            ...
            EndAt = request.StartAt.AddMinutes(hairdresserService.DurationMinutes),
        ...
        await unitOfWork.ReservationRepository.CreateForAdminAsync(reservation, ct);
