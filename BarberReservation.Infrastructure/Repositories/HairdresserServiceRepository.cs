...
public sealed class HairdresserServiceRepository(BarberDbContext context) : IHairdresserServiceRepository
{
    ...
    public async Task<(IReadOnlyList<HairdresserService>, int)> GetAllPagedForAdminAsync(int page, int pageSize, string? hairdresserId, int? serviceId, string? sortBy, bool desc, bool? isActive, CancellationToken ct)
    {
        page = page <= 0 ? 1 : page;
